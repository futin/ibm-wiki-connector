    ...
    if (versionLabel) {
      ...
      service.getPagesFeed(options)
        .then((data) => { // eslint-disable-line consistent-return
          if (data && _.isArray(data.entries)) {
            const lastEntry = data.entries[data.entries.length - 1];
            // assign new property to options that is going to be parsed by wiki service
            // the reason we need last entry is because atom is stacking pages in FILO method, so in order to
            // display first element in UI, we need last one from our array
            options = _.assign(options, { pageLabel: lastEntry.id }); // eslint-disable-line no-param-reassign
            Promise.all(collectPromises(service, options, filter))
              .then((res) => {
                // switch last entry with data from whole wiki page, not just 'id' and 'title'
                data.entries[data.entries.length - 1] = parseData(res); // eslint-disable-line no-param-reassign
                const result = _.assign(filter, data);
                return callback(null, result);
              })
              .catch(err => callback(err));
          } else {
            return callback(new Error('There should be {{ entries }} array in returned data'));
          }
        })
        .catch(err => callback(err));
    } else {
