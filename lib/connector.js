...
const IbmConnectionsWikisService = require('ibm-connections-wikis');
...
function wikiConnector(settings, dataSource) {
  ...
  function connect(callback) {
    ...
    process.nextTick(() => callback(null, connector));
  ...
  function all(model, filter, options, callback) { // eslint-disable-line consistent-return
    ...
    if (versionLabel) {
      const promises = [];
      const modifiedOptions = _.assign(options, { showLinkOpenSource: filter.showLinkOpenSource });
      promises.push(service.getPageVersion(modifiedOptions));
      ...
      // 3. wikiLabel alone -> wikis/authPath/api/wiki/${options.wikiLabel}/nav/feed
    } else if (wikiLabel) {
      service.getNavigationFeed(options)
        .then((result) => {
